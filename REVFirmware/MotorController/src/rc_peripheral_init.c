...
void peripheral_init(void)
{
...
#ifdef PAC5556
	// Configure Timer B parameters. Timer A runs at PLL Frequency (selected above).
	...
	pac5xxx_timer_base_config(TimerB, pwm_period_ticks, 0, TxCTL_MODE_UP, 0);	// Configure Timer
#else
	...
	PAC55XX_GPIOB->MODE.w = MOTOR_OUTPUTS_PUSH_PULL_B;
	PAC55XX_GPIOC->MODE.w = MOTOR_OUTPUTS_PUSH_PULL_C;
#else
	PAC55XX_GPIOB->OUT.w = 0;			//PORTB OUT GPIO = 0;
	PAC55XX_SCC->PBMUXSEL.w = 0;		//PORTB PSEL is ALL GPIO's
	//Configure PORTB Outputs to Push Pull
	...
	// Configure Timer B clock input for HCLK source and /32 divider
	...
	// Timer B in COUNT UP Mode up to max value (65535d = 0xFFFF)
	// Timer B CCTR0 configure to CAPTURE and INTERRUPT on both RISE and FALL edges
	// Timer B Base interrupt is used as a Watchdog Timer to ensure RC transmitter is in place
	pac5xxx_timer_clock_config(TimerB, TXCTL_CS_PCLK, TXCTL_PS_DIV32);
	PAC55XX_TIMERB->CTL.PRDLATCH = TXCTL_PRDLATCH_TXCTR_IM;
	PAC55XX_TIMERB->CCTL7.CCMODE = TXCCTL_CCMODE_CAPTURE;
	PAC55XX_TIMERB->CCTL7.CCINTEDGE = TXCCTL_CCINT_BOTH_EDGES;
	PAC55XX_TIMERB->CCTL7.CCINTEN = 1;
	PAC55XX_TIMERB->CCTL7.CCLATCH = TXCCTL_CCLATCH_CAPTURE_BOTH_EDGES;
	PAC55XX_TIMERB->CTL.BASEIE = 1;
	pac5xxx_timer_base_config(TimerB, 0xFFFF, 0, TxCTL_MODE_UP, 0);
	NVIC_SetPriority(TimerB_IRQn, 1);
	NVIC_EnableIRQ(TimerB_IRQn);

#if defined (PAC5523) || (PAC5524)
	// PC7 is configured as PWMB7 - RC PWM signal input
	PAC55XX_GPIOC->MODE.P7 = IO_HIGH_IMPEDENCE_INPUT;
	PAC55XX_SCC->PCMUXSEL.P7 = 1;						//DPM PC7 to TIMERB7
#elif defined (PAC5527)
	// PD7 is configured as PWMB7 - RC PWM signal input
	PAC55XX_GPIOD->MODE.P7 = IO_HIGH_IMPEDENCE_INPUT;
	PAC55XX_SCC->PDMUXSEL.P7 = 1;						//DPM PD7 to TIMERB7
#endif
