		...
		case cmd_parameter_write:
			...
			case parm_pwm_period:
				...
				if (rx_data != app_pwm_period)
				{
					...
					pwm_period_ticks = TIMER_X_FREQ_CNV / (app_pwm_period);			//100,000 / # KHz
			...
			case parm_hp_over_current_limit:
				...
				pac5xxx_tile_register_write(ADDR_HPDAC, hp_over_current_limit);
				break;
			case parm_lp_over_current_limit:
				lp_over_current_limit = rx_data;
				pac5xxx_tile_register_write(ADDR_LPDAC0, lp_over_current_limit >> 2);	// LPDAC: LPROT DAC (8b)
				pac5xxx_tile_register_write(ADDR_LPDAC1, lp_over_current_limit & 0x03);	// LPDAC: LPROT DAC (1b)
				break;
#else
			case parm_hp_over_current_limit:
				hp_over_current_limit = rx_data;
			...
			case parm_enable_speed_pi:
				...
				if (enable_speed_pi)
					...
					app_status &= ~status_speed_pi_enabled;
				break;
			case parm_enable_current_pi:
				enable_current_pi = rx_data;
